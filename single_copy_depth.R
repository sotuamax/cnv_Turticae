...
parse_arg <- function() {
    ...
    parser$add_argument("-RD", "--read_depth", help = "mean and median of read-depth per gene. ")
    ...
    parser$add_argument("-right_disperse", "--right_disperse", default = 0.55, help = "disperse range of read-depth on the right side of the peak-value. ")
...
p <- ggplot(data = hist_freq) + geom_col(aes(x = bar_mid, y = bar_prop)) + geom_line(aes(x = bar_mid, y = exp_prop), col = "red") + geom_vline(xintercept = mode_lower, col = "red", linetype = "dashed") + geom_vline(xintercept = mode_upper, col = "red", linetype = "dashed") + geom_vline(xintercept = mu, col = "red", linetype = "dashed") +  xlim(c(0, mu*4)) + theme_bw()
